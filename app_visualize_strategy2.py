...
def plot_multi_candlesticks(stock_info_dict):
    ...
    fig = make_subplots(
        rows=n_stocks, cols=1, shared_xaxes=True,
        vertical_spacing=0.03,
        subplot_titles=list(stock_info_dict.keys())
    )
...
def run():
    ...
    if selected_foldername is not None:
        ...
        if "last_input_timestamp" not in st.session_state or st.session_state.last_input_timestamp != input_timestamp:
            st.session_state.analysis_timestamp_id = np.searchsorted(
                st.session_state.available_backtest_timestamps, input_timestamp
            )
        ...
        spot_dict = {'dataframe': df_spot_filtered, 'redgreen_timestamp_start': df_spot_filtered.index[0], 'redgreen_timestamp_end': df_spot_filtered.index[-1],}
        ...
        for idx, row in df_position_info.iterrows():
            ...
            df_option = DBConnector().get_option_df(
            ...
            position_info_dict['vlines_timestamp_to_color_map'] = {
                row['pos_entry_ts']: "lightpink" if action.trade_type == "short" else "palegreen",
                current_timestamp: "lightgrey"
            }
        ...
        display_text_container(text=f"TOTAL PnL ACROSS POSITIONS ::::: {total_pnl:.4f}",
                               border_color="crimson" if total_pnl < 0 else "green", height=120, font_size=28)
