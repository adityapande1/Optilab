...
def plotly_stem(hash2position_dfs, backtest_dir):
    ...
    fig = make_subplots(rows=n_plots, cols=1, shared_xaxes=True, vertical_spacing=0.05, subplot_titles=titles)

    # --- Combined PnL (row 1) ---
    for t, v in zip(combined_pnl.index, combined_pnl.values):
        if pd.notna(v):
            color = 'green' if v > 0 else 'red'
            fig.add_trace(go.Scatter(x=[t, t], y=[0, v], mode='lines', line=dict(color=color)), row=1, col=1)
            fig.add_trace(go.Scatter(x=[t], y=[v], mode='markers', marker=dict(color=color, size=6)), row=1, col=1)
    ...
    for i, (h, df) in enumerate(aligned_dfs.items(), start=2):  # start=2 because row=1 is combined
        for t, v in zip(df.index, df['pnl']):
            if pd.notna(v):
                color = 'green' if v > 0 else 'red'
                fig.add_trace(go.Scatter(x=[t, t], y=[0, v], mode='lines', line=dict(color=color)), row=i, col=1)
                fig.add_trace(go.Scatter(x=[t], y=[v], mode='markers', marker=dict(color=color, size=6)), row=i, col=1)

    fig.update_layout(height=300 * n_plots, showlegend=False, title_text='PnL Stem Plots', title_x=0.5)
