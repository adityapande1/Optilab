...
class BaselineStrangle(Strategy):
    ...
    def about(self) -> str:
        ...
        about_str += f"     left_strike : [OTM Put Strike] = center_strike - {self.strike_gap} X {self.left_strike_gap_multiple}\n"
        about_str += f"         {self.long_or_short.upper()}: [OTM PUT] of left_strike (X1)\n"
        about_str += f"     right_strike : [OTM Call Strike] = center_strike + {self.strike_gap} X {self.right_strike_gap_multiple}\n"
        about_str += f"         {self.long_or_short.upper()}: [OTM CALL] of right_strike (X1)\n"
