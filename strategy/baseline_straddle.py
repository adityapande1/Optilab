    ...
    def action(self, timestamp: pd.Timestamp) -> list[Action] | None:
        ...
        if timestamp.time() == self.entry_timestamp.time(): 
            ...
            if self.long_or_short == "short":
                short_atm_call = Action(option_type="CE", strike=self.strike, expiry=closest_expiry, num_lots=1, trade_type="sell", order_type="market")
                short_atm_put = Action(option_type="PE", strike=self.strike, expiry=closest_expiry, num_lots=1, trade_type="sell", order_type="market")
                actions = [short_atm_call, short_atm_put]
            elif self.long_or_short == "long":
                long_atm_call = Action(option_type="CE", strike=self.strike, expiry=closest_expiry, num_lots=1, trade_type="buy", order_type="market")
                long_atm_put = Action(option_type="PE", strike=self.strike, expiry=closest_expiry, num_lots=1, trade_type="buy", order_type="market")
