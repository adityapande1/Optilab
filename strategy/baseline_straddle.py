...
class BaselineStraddle(Strategy):
    def __init__(self, 
                 config, 
    ...
    def _has_stoploss_or_target_hit(self, timestamp: pd.Timestamp) -> bool:
        ...
        stoploss_hit = (pnl <= -self.config.risk_per_trade)     # Note: StrategyConfig.risk_per_trade is asserted > 0
    ...
    def action(self, timestamp: pd.Timestamp) -> list[Action] | None:
        ...
        if timestamp.time() == self.entry_timestamp.time(): 
            ...
            if self.long_or_short == "short":
                short_atm_call = Action(option_type="CE", strike=self.strike, expiry=closest_expiry, num_lots=1, trade_type="short", order_type="market")
                short_atm_put = Action(option_type="PE", strike=self.strike, expiry=closest_expiry, num_lots=1, trade_type="short", order_type="market")
                actions = [short_atm_call, short_atm_put]
            elif self.long_or_short == "long":
                long_atm_call = Action(option_type="CE", strike=self.strike, expiry=closest_expiry, num_lots=1, trade_type="long", order_type="market")
                long_atm_put = Action(option_type="PE", strike=self.strike, expiry=closest_expiry, num_lots=1, trade_type="long", order_type="market")
                actions = [long_atm_call, long_atm_put]
    ...
    def about(self) -> str:
        ...
        about_str += f"     Enter an Straddle of (nearest) ATM at {self.entry_timestamp.time().strftime('%H:%M:%S')} at close of underlying\n"
        if self.long_or_short == "short":
            opposite_pos = "LONG"
        elif self.long_or_short == "long":
            opposite_pos = "SHORT"
        about_str += f"     Net position: \n"
        about_str += f"         {self.long_or_short.upper()}: [ATM Call and ATM Put] of strike according to close at {self.entry_timestamp.time().strftime('%H:%M:%S')} (X2)\n"
