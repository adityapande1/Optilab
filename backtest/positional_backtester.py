...
#     def _get_df_stoploss_for_individual_order_wrt_overall_position(self, order_stats: dict, df_stoploss_overall_position: pd.DataFrame) -> pd.DataFrame:
#         """
...
#         df_option = self.dbconnector.get_option_df(option_type=order_stats['action'].option_type, strike=order_stats['action'].strike, expiry_date=order_stats['action'].expiry)[
#             ['open', 'high', 'low', 'close']
#         ]
...
#             max_possible_exit_timestamp = pd.Timestamp.combine(entry_timestamp.date(), self.strategy.config.exit_time)
#         else:
...
#         overall_position_stoploss_hit_timestamp = None
#         if not df_stoploss_overall_position.empty and df_stoploss_overall_position.iloc[-1]['stoploss_hit']:
#             overall_position_stoploss_hit_timestamp = df_stoploss_overall_position.index[-1]

#         if overall_position_stoploss_hit_timestamp:
#             exit_timestamp = min(max_possible_exit_timestamp, overall_position_stoploss_hit_timestamp)
#         else:
...
#         df_option['stoploss_hit'] = False
#         df_option['price'] = df_option['close']
#         df_stoploss = df_option.copy()

#         if exit_timestamp == overall_position_stoploss_hit_timestamp:
#             assert df_stoploss.index[-1] == overall_position_stoploss_hit_timestamp, 'DataFrame last index must match overall position stoploss hit timestamp'
#             df_stoploss.at[overall_position_stoploss_hit_timestamp, 'stoploss_hit'] = True

#         return df_stoploss

#     def initialize_stoploss_dataframes_for_each_individual_order(self, filled_orders: list[dict], df_stoploss_overall_position: pd.DataFrame) -> set:
#         """
...
#             ):
#                 df_stoploss = self._get_df_stoploss_for_individual_order_wrt_overall_position(order_stats, df_stoploss_overall_position)
...
#             df_stoploss_combined = self.calculate_stoploss_levels(
#                 df=df_option_combined, starting_stoploss_level=starting_stoploss_level, position_type='long', trail_stoploss=self.config.trail_total_position_risk
#             )
...
#             # 3.1.1
#             df_stoploss_overall_position = self._get_combined_stoploss_dataframe(filled_orders)
#             self.initialize_stoploss_dataframes_for_each_individual_order(filled_orders, df_stoploss_overall_position)
