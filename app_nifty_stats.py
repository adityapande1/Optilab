...
def plot_histogram(df: pd.DataFrame, colname: str, height: int = 600, nbins: int = 200, tick_size: int = 28):
    fig = px.histogram(df, x=colname, nbins=nbins, marginal='box', hover_data=df.columns, title=f'Histogram of {colname}', color_discrete_sequence=['#6495ed'], height=height)
    ...
    fig.update_layout(
        ...
        bargap=0.1,
        hovermode='x unified',
        xaxis=dict(tickfont=dict(size=tick_size), gridcolor='lightgray', gridwidth=0.5, showgrid=True),
        yaxis=dict(tickfont=dict(size=tick_size), gridcolor='lightgray', gridwidth=0.5, showgrid=True),
    )
    fig.update_traces(marker_line_width=1.5, marker_line_color='black', opacity=1.0, selector=dict(type='histogram'))
    ...
    fig.add_annotation(x=mean_val, y=1.1, yref='paper', text=f'Mean (μ) <br> {mean_val:.4f}', showarrow=False, font=dict(color='black', size=int(tick_size * 0.8)))
    fig.add_annotation(x=mean_val - sigma, y=1.1, yref='paper', text=f'μ - 1σ <br> {mean_val - sigma:.4f}', showarrow=False, font=dict(color='black', size=int(tick_size * 0.8)))
    fig.add_annotation(x=mean_val + sigma, y=1.1, yref='paper', text=f'μ + 1σ <br> {mean_val + sigma:.4f}', showarrow=False, font=dict(color='black', size=int(tick_size * 0.8)))
    fig.add_annotation(x=mean_val + 2 * sigma, y=1.1, yref='paper', text=f'μ + 2σ <br> {mean_val + 2 * sigma:.4f}', showarrow=False, font=dict(color='black', size=int(tick_size * 0.8)))
    fig.add_annotation(x=mean_val - 2 * sigma, y=1.1, yref='paper', text=f'μ - 2σ <br> {mean_val - 2 * sigma:.4f}', showarrow=False, font=dict(color='black', size=int(tick_size * 0.8)))
...
def run():
    ...
    selected_start_date = st.sidebar.date_input('Start Date', value=unique_dates[0].date(), min_value=unique_dates[0].date(), max_value=unique_dates[-1].date())
    min_end_date = unique_dates[unique_dates >= pd.to_datetime(selected_start_date)][0]
    selected_end_date = st.sidebar.date_input('End Date', value=unique_dates[-1].date(), min_value=min_end_date.date(), max_value=unique_dates[-1].date())
    df_nifty_daily = df_nifty_daily[(df_nifty_daily.index >= pd.to_datetime(selected_start_date)) & (df_nifty_daily.index <= pd.to_datetime(selected_end_date))]
