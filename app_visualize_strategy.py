...
def shift_indices(step: int, df_spot):
    st.session_state.start_idx = min(max(0, st.session_state.start_idx + step), len(df_spot) - 1)
    st.session_state.end_idx = min(max(0, st.session_state.end_idx + step), len(df_spot) - 1)
...
def get_position_infos(backtest_analyzer):
    ...
    for position_hash in backtest_analyzer.get_position_hashes():
        ...
        dictio = {'position_hash': position_hash, 'first_timestamp': df_position.index.min(), 'last_timestamp': df_position.index.max()}
...
def make_candlestick_subplot(df_list, titles=None, height_per_chart=400):
    ...
    fig = make_subplots(rows=n, cols=1, shared_xaxes=True, vertical_spacing=0.01, subplot_titles=titles)
